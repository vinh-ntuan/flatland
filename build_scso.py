...
from scso.scso_rail_env import ScsoRailEnv
    ...
    for param, expected_type in required_params.items():
        if not hasattr(par, param):
            ...
            if expected_type is int and isinstance(value, bool):
                raise TypeError(
                    f"Parameter '{param}' cannot be a boolean; it should be of type {expected_type.__name__}.")

            if not isinstance(value, expected_type):
                raise TypeError(
                    f"Parameter '{param}' should be of type {expected_type.__name__}, but got {type(value).__name__}")
...
def get_args():
    ...
    parser.add_argument('num_envs', type=int, default=1, nargs='?',
                        help='the number of environments to create according to the given parameters')
